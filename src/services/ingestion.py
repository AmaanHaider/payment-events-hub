...
from src.domain import TERMINAL_PAYMENT_EVENT_TYPES, EventType, PaymentStatus
...
def apply_payment_lifecycle(txn: Transaction, event: EventIn) -> None:
    try:
        et = event.event_type
...
def refresh_reconciliation_flags(txn: Transaction) -> None:
    _refresh_reconciliation_flags(txn)


def reset_transaction_derived_state(txn: Transaction) -> None:
    """
    Clear derived reconciliation/payment fields so they can be recomputed from event history.
    """
    txn.payment_status = None
    txn.last_payment_event_at = None
    txn.last_payment_event_id = None

    txn.terminal_payment_status = None
    txn.terminal_payment_event_at = None
    txn.terminal_payment_event_id = None
    txn.payment_conflict = False

    txn.has_settlement = False
    txn.settled_at = None
    txn.settlement_event_id = None

    txn.recon_processed_not_settled = False
    txn.recon_settled_without_processed = False
    txn.recon_settled_after_failed = False
...
def _conflict_fields(stored: Event, incoming: EventIn) -> list[str]:
    fields: list[str] = []
    if stored.event_type != incoming.event_type.value:
        fields.append("event_type")
    if stored.transaction_id != incoming.transaction_id:
        fields.append("transaction_id")
    if stored.merchant_id != incoming.merchant_id:
        fields.append("merchant_id")
    if float(stored.amount) != float(incoming.amount):
        fields.append("amount")
    if stored.currency != incoming.currency:
        fields.append("currency")
    if normalize_ts(stored.occurred_at) != normalize_ts(incoming.timestamp):
...
def ingest_payment_event(db: Session, event_in: EventIn) -> IngestResult:
    ...
    stored = db.get(Event, event_in.event_id)
    if stored is not None:
        conflicts = _conflict_fields(stored, event_in)
        if conflicts:
            return IngestResult(
                accepted=False,
                duplicate=True,
    ...
    # Wrap the entire ingest in a savepoint so a duplicate insert detected at flush-time
    # doesn't leave the Session in a partial state.
    try:
        with db.begin_nested():
            ...
            # Serialize lifecycle updates for the same transaction (Postgres).
            db.flush()
            bind = db.get_bind()
            ...
            if dialect == "postgresql":
                locked = db.scalars(
                    select(Transaction)
                    .where(Transaction.transaction_id == event_in.transaction_id)
                    .with_for_update()
                ).one()
                if locked is not txn:  # pragma: no cover
                    txn = locked
            ...
            # Force INSERTs now so duplicates are caught here, not at commit().
            db.flush()

            apply_payment_lifecycle(txn, event_in)
